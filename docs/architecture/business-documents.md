...
POS-da inzibatçı endpointləri Bearer credential və filial konteksti ilə işləyir. `/api/v1/pos/sync/*` cihaz endpointləri isə ayrıca `Authorization: Device <tenant-uuid>.<device-secret>` credentialı ilə qorunur. Aktivləşdirmə, bootstrap, push, pull və ack əməliyyatları offline cihazın əməliyyat jurnalını ERP-yə sinxronlaşdırır; POS satış, qaytarış, növbə və kassa hərəkətləri sonradan ERP oxu endpointlərində görünür.
