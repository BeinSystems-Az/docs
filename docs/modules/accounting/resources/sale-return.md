---
sidebar_position: 23
title: Satış qaytarışları
---

# Satış qaytarışları

Satış qaytarışları resursu mühasibatlıq və maliyyə modulunda aid olduğu məlumat və əməliyyatları idarə edir.

:::info Kontekst
`Bearer` JWT və ya integration token · tenant və filial scope-u · `sale_returns`
:::

## Resursun işləmə qaydası

**Məqsəd və sərhəd.** Satış qaytarışları resursu mühasibatlıq və maliyyə modulunda aid olduğu məlumat və əməliyyatları idarə edir. Bu modul maliyyə nəticəsini və baş kitabı idarə edir; fiziki stok hərəkətinin mənbəyi satış, alış və stok sənədləridir.
...
### Satış qaytarışları siyahısını al

**Endpoint** · `GET /api/v1/sale-returns`

Cari istifadəçi və scope daxilində məlumatı dəyişiklik etmədən qaytarır.

**İcazə** · `sale_returns.read`
...
### Satış qaytarışları qeydi yarat

**Endpoint** · `POST /api/v1/sale-returns`

Sorğudakı məlumatla yeni qeyd və ya resursa aid domain əməliyyatı yaradır.

**İcazə** · `sale_returns.create`
...
### Satış qaytarışları state-ni dəyiş

**Endpoint** · `PATCH /api/v1/sale-returns/{entry}/state`

Resursun lifecycle vəziyyətini backend keçid qaydalarına uyğun dəyişir.

**İcazə** · `sale_returns.__document_transition__`
...
### Satış qaytarışları qeydini sil

**Endpoint** · `DELETE /api/v1/sale-returns/{sale_return}`

Qeydi backend-in silmə və bağlı məlumat məhdudiyyətlərinə uyğun arxivləşdirir və ya silir.

**İcazə** · `sale_returns.delete`
...
```json
{
  ...
  "path": {
    "sale_return": "22222222-2222-4222-8222-222222222222"
  },
  "query": {},
  "body": {}
}
...
### Satış qaytarışları qeydini oxu

**Endpoint** · `GET /api/v1/sale-returns/{sale_return}`
...
### Satış qaytarışları qeydini yenilə

**Endpoint** · `PATCH|PUT /api/v1/sale-returns/{sale_return}`

Mövcud qeydin göndərilən sahələrini validation və domain qaydalarına uyğun yeniləyir.

**İcazə** · `sale_returns.update`
