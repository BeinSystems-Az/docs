---
sidebar_position: 8
title: Birbaşa xərclər
---

# Birbaşa xərclər

Birbaşa xərclər resursu mühasibatlıq və maliyyə modulunda aid olduğu məlumat və əməliyyatları idarə edir.

:::info Kontekst
`Bearer` JWT və ya integration token · tenant və filial scope-u · `direct_expenses`
:::

## Resursun işləmə qaydası

**Məqsəd və sərhəd.** Birbaşa xərclər resursu mühasibatlıq və maliyyə modulunda aid olduğu məlumat və əməliyyatları idarə edir. Bu modul maliyyə nəticəsini və baş kitabı idarə edir; fiziki stok hərəkətinin mənbəyi satış, alış və stok sənədləridir.
...
| `landed_cost_target_receipt_id` | UUID | Qeydi əlaqəli “landed cost target receipt” resursuna bağlayır. |
| `landed_cost_allocation_method` | enum/string | Resursun “landed cost allocation method” məlumatını saxlayır və uyğun əməliyyatlarda istifadə olunur. |
| `items` | array/object | Sənədin məhsul, hesab və ya əməliyyat sətirləridir. |
| `items[].expense_category_id` | UUID | Qeydi əlaqəli “expense category” resursuna bağlayır. |
| `items[].amount` | number | Əməliyyatın pul məbləğidir. |
| `items[].description` | array field | Qeyd haqqında əlavə izahdır. |

## Endpointlər

### Birbaşa xərclər siyahısını al

**Endpoint** · `GET /api/v1/direct-expenses`

Cari istifadəçi və scope daxilində məlumatı dəyişiklik etmədən qaytarır.

**İcazə** · `direct_expenses.read`
...
{
  "status": "success",
  "message": "Xərclər uğurla siyahılandı.",
  "data": [
    {
      "name": "Nümunə qeyd",
      "state": "draft",
      ...
      "landed_cost_target_receipt_id": "33333333-3333-4333-8333-333333333333"
    }
  ],
  "links": {
...
### Birbaşa xərclər qeydi yarat

**Endpoint** · `POST /api/v1/direct-expenses`

Sorğudakı məlumatla yeni qeyd və ya resursa aid domain əməliyyatı yaradır.

**İcazə** · `direct_expenses.create`
...
### Birbaşa xərclər qeydini sil

**Endpoint** · `DELETE /api/v1/direct-expenses/{direct_expense}`

Qeydi backend-in silmə və bağlı məlumat məhdudiyyətlərinə uyğun arxivləşdirir və ya silir.

**İcazə** · `direct_expenses.delete`
...
```json
{
  ...
  "path": {
    "direct_expense": "22222222-2222-4222-8222-222222222222"
  },
  "query": {},
  "body": {}
}
...
### Birbaşa xərclər qeydini oxu

**Endpoint** · `GET /api/v1/direct-expenses/{direct_expense}`
...
### Birbaşa xərclər qeydini yenilə

**Endpoint** · `PATCH|PUT /api/v1/direct-expenses/{direct_expense}`

Mövcud qeydin göndərilən sahələrini validation və domain qaydalarına uyğun yeniləyir.

**İcazə** · `direct_expenses.update`
...
### Birbaşa xərclər state-ni dəyiş

**Endpoint** · `PATCH /api/v1/direct-expenses/{expense}/state`

Resursun lifecycle vəziyyətini backend keçid qaydalarına uyğun dəyişir.

**İcazə** · `direct_expenses.__document_transition__`
...
{
  "status": "success",
  "message": "Xərcin vəziyyəti yeniləndi.",
  "data": {
    "name": "Nümunə qeyd",
    "state": "draft",
    "branch_id": "44444444-4444-4444-8444-444444444444",
